...
## Done
- ST-001 storage follow-through (luna, 2026-09-11): Spark 2.0T→413G used; 104 evidence dirs + 5 golden runs promoted to `llm_work` (symlinks left), 6 HF models → `s3://llm-models/hf/`, sglang image → `s3://artifacts/docker/`, biocoder private → `s3://artifacts/biocoder/`, weekly `_trash` purge + `tier-df` cron. Verified by orchestrator: 19/19 cited paths resolve, serving container up.
