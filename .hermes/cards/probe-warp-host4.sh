#!/usr/bin/env bash
# PROBE only: verified build plus both variants on supported batches.
set -euo pipefail
CUTOFF=1788750000
LEFT=$((CUTOFF-$(date +%s)))
((LEFT>0)) || exit 124
...
ROOT="$LAB/probe-source-v4"
OUT="$LAB/probe-warp-host4"
mkdir "$OUT"
...
run warp "$BIN" run "${COMMON[@]}" --output "$OUT/probe-warp.tsv" --variant warp --pages huge --counts 64000,256000,1000000,4000000
...
out=Path(sys.argv[1])
for name,expected in [('warp',24),('thread',24),('warp-small4k',6)]:
